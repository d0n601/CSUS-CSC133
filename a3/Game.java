...
public class Game extends Form implements Runnable {
	...
	public Game() {
		...
		gw.addObserver(this.gwp); // Our Game World Proxy observes the game world.
		gwp.addObserver(this.mv); // Add the Map View as an observer of the Game World Proxy.
		gwp.addObserver(this.pv); // Add the Points as an observer of the Game World Proxy.
		
		this.timer = new UITimer(this);
		this.timer.schedule(20, true, this);
		...
		spaceToolbar.setTitle("Asteroid Game (ver.a3)");
		...
		PauseGameCommand pauseGameCommand = new PauseGameCommand(gw);
		...
		spaceToolbar.addCommandToOverflowMenu(pauseGameCommand);
	...
	public void run() {
		...
		if(!this.gw.isPaused()) {
			...
			this.gw.handleCollisions();
		}

	
		this.r = new Random();
		/*
		// A .25% chance we'll add an NPS this cycle.
		if(this.r.nextInt(400) < 1 ) {
			this.gw.addNps();
		}
		*/
		
		// A 1% chance we'll add an Asteroid.
		if(this.r.nextInt(100) < 1 ) {
			this.gw.addAsteroid();
		}
